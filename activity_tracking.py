...
#		sizer.Add( self.panel, 1, wx.EXPAND |wx.ALL, 0 )
	...
	def OnUpdate(self, evt):
		...
		cond = sim_thread.isAlive() if hasattr(sim_thread,'isAlive') else sim_thread.is_alive()
...
def Config(parent):
	...
	frame = wx.Frame(parent, wx.ID_ANY, title = _('Activity Tracking Settings - %s')%diagram.GetName(), style = wx.DEFAULT_FRAME_STYLE | wx.CLIP_CHILDREN | wx.STAY_ON_TOP)
	...
	if L1:
		...
		### tout les blocks on la meme liste de function active pour le trace, donc on prend la première
	...
	def OnOk(evt):
		...
		for index in range(num1):
			...
			if cb1.IsChecked(index):
				...
				D = dict([(i, cb2.GetString(i)) for i in range(num2) if cb2.IsChecked(i)])
